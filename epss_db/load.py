...
def _write_chunk(ddb_table, chunk: List[Dict[str, Any]]):
    with ddb_table.batch_writer() as batch:
        for rec in chunk:
            item = {k: _to_ddb_safe(v) for k, v in rec.items() if _to_ddb_safe(v) is not None}
...
def sync_epss_records_to_dynamodb_and_s3(records: List[Dict[str, Any]], json_bytes: bytes, user_cfg: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for chunk in [to_write[i:i + cfg["BATCH_WRITE_CHUNK_SIZE"]] for i in range(0, len(to_write), cfg["BATCH_WRITE_CHUNK_SIZE"])]:
